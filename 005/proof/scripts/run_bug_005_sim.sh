#!/usr/bin/env bash
# BUG-005 directed simulation: unit-level aes_reg_top over TL-UL.
# Confirms that the write-only DATA_IN registers return software-written
# plaintext on a read, for all four words, repeatably.
set -euo pipefail
...
# instantiations from aes_reg_top -> adapter_reg / cmd_intg_chk -> leaf cells.
...
echo "$TB/aes_reg_bug_005_tb.sv" >> filelist.f
...
vcs -full64 -sverilog -timescale=1ns/1ps \
    -assert svaext \
    -f filelist.f \
    -top aes_reg_bug_005_tb \
    -l "$CMP_LOG" -o simv 2>&1 | tail -5

./simv -l "$SIM_LOG" 2>&1 | tail -45
...
grep -E 'cover_data_in_leak|cover_legal_read|recovered 128-bit' "$SIM_LOG" || true
