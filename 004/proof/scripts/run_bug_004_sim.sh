...
#
# BUG-004 witness simulation. Elaborates one aes_reg_top inside the project's own
# aes filelist and runs the register-bus witness testbench.
#
...
#
# DUT_AES_REG_TOP lets the negative control substitute a patched scratch copy of
# aes_reg_top.sv. Unset, it resolves to the in-tree file, so this script is the
# unmodified-tree run.
set -euo pipefail
...
TB="$CASE/proof/tb/aes_bug_004_tb.sv"

DUT_AES_REG_TOP="${DUT_AES_REG_TOP:-$CMP/src/aes/rtl/aes_reg_top.sv}"
...
mkdir -p "$LOGS"
BUILD="$(mktemp -d "${TMPDIR:-/tmp}/bug004_sim.XXXXXX")"
cleanup() { rm -rf "$BUILD"; }
trap cleanup EXIT
...
python3 - "$CMP/src/aes/config/aes.vf" "$FLIST" "$CMP" "$DUT_AES_REG_TOP" <<'PY'
import os, sys
...
for raw in open(src):
    ...
    if os.path.basename(line) == "aes_reg_top.sv":
        line = dut
...
echo "DUT_AES_REG_TOP=$DUT_AES_REG_TOP" | tee "$LOGS/dut_selection.log"
...
if grep -q "PROOF_RESULT: PASS" "$LOGS/run.log"; then
  echo "BUG-004 witness: PASS"
  exit 0
fi
echo "BUG-004 witness: FAIL (sim rc=$SRC_RC)"
exit 1
