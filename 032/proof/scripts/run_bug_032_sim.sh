#!/usr/bin/env bash
# BUG-032 directed simulation: unit-level sha256 with its own register block.
#
# Drives both values of debugUnlock_or_scan_mode_switch and reports what the
# SHA256_DIGEST window actually does in each case, then asserts the two
# security-relevant conclusions.
#
# Uses the project's own filelist (src/sha256/config/sha256_ctrl.vf) rather than a
...
DUT_SHA256="${DUT_SHA256:-$CMP/src/sha256/rtl/sha256.sv}"
...
python3 - "$CMP/src/sha256/config/sha256_ctrl.vf" "$DUT_SHA256" > filelist.f <<'PY'
import os, sys
...
canonical_dut = os.path.join(root, "src/sha256/rtl/sha256.sv")
for line in open(vf):
    line = line.strip()
...
echo "$TB/sha256_bug_032_tb.sv" >> filelist.f
