...
#
# Build and run the kv_bug_023 directed simulation, then the structural audit.
#
# Compiles exactly one DUT (src/keyvault/rtl/kv.sv, with its register block and
# AHB shim) and drives it from proof/tb/kv_lock_enforcement_tb.sv.
...
SRCS=(
  ...
  "${TB_DIR}/kv_lock_enforcement_tb.sv"
)
INCDIRS=(
...
echo "=== kv_bug_023 proof run ===" > "${LOG_DIR}/run.log"
echo "dut=src/keyvault/rtl/kv.sv" >> "${LOG_DIR}/run.log"
echo "tb=proof/tb/kv_lock_enforcement_tb.sv" >> "${LOG_DIR}/run.log"
...
sim_rc=0
./kv_bug_023_sim -no_save >> "${LOG_DIR}/sim.log" 2>&1 || sim_rc=$?
...
{
  echo "=== kv_bug_023 measured decisions ==="
  grep -E '^(CONTROL_A|CONTROL_B|CONTROL_C|CONTROL_D|PROBE_A|PROBE_B|ok:|FAIL:|checks_run|probe_)' \
...
# ---------------------------------------------------------------------------
audit_rc=0
CALIPTRA_ROOT="${REPO_ROOT}" "${SCRIPT_DIR}/audit_kv_lock_set_sources.sh" \
