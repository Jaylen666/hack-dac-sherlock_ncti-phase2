#!/usr/bin/env bash
# BUG-029 directed simulation: unit-level sha3.
#
# Uses the project's own filelist (src/kmac/config/kmac.vf) rather than a
...
DUT_SHA3="${DUT_SHA3:-$CMP/src/kmac/rtl/sha3.sv}"
SIM_LOG="${SIM_LOG:-$LOGS/sim.log}"
CMP_LOG="${CMP_LOG:-$LOGS/compile.log}"
TB_FILE="${TB_FILE:-$TB/sha3_bug_029_tb.sv}"
TB_TOP="${TB_TOP:-sha3_bug_029_tb}"
...
python3 - "$CMP/src/kmac/config/kmac.vf" "$DUT_SHA3" > filelist.f <<'PY'
import os, sys
...
canonical_dut = os.path.join(root, "src/kmac/rtl/sha3.sv")
for line in open(vf):
    line = line.strip()
...
# kmac.vf lists the caliptra_prim packages but not the prim modules that sha3
# and keccak_round instantiate: the sparse-FSM flop behind the
# CALIPTRA_PRIM_FLOP_SPARSE_FSM macro, the security anchor buffer that gates the
# storage reset, and the hardened round counter. Each is named explicitly rather
# than globbed, because the prim directory also holds cells that depend on
# packages outside this block's closure. All are verbatim tree files.
echo "+incdir+$CMP/src/caliptra_prim_generic/rtl" >> filelist.f
for m in caliptra_prim_buf \
         caliptra_prim_flop \
         caliptra_prim_sec_anchor_buf \
         caliptra_prim_sparse_fsm_flop \
         caliptra_prim_count \
         caliptra_prim_slicer; do
  echo "$CMP/src/caliptra_prim/rtl/$m.sv" >> filelist.f
done
for m in caliptra_prim_generic_buf caliptra_prim_generic_flop; do
  echo "$CMP/src/caliptra_prim_generic/rtl/$m.sv" >> filelist.f
done
