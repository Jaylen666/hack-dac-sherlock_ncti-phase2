...
#
# Build and run the kv_bug_022 directed simulation, then the structural audit.
#
# Compiles exactly one DUT (src/keyvault/rtl/kv.sv, with its register block and
# AHB shim) and drives it from proof/tb/kv_stage_transition_retention_tb.sv.
...
SRCS=(
  ...
  "${TB_DIR}/kv_stage_transition_retention_tb.sv"
)
INCDIRS=(
...
echo "=== kv_bug_022 proof run ===" > "${LOG_DIR}/run.log"
echo "dut=src/keyvault/rtl/kv.sv" >> "${LOG_DIR}/run.log"
echo "tb=proof/tb/kv_stage_transition_retention_tb.sv" >> "${LOG_DIR}/run.log"
...
sim_rc=0
./kv_bug_022_sim -no_save >> "${LOG_DIR}/sim.log" 2>&1 || sim_rc=$?
...
{
  echo "=== kv_bug_022 measured decisions ==="
  grep -E '^(SETUP|PROBE_A|PROBE_B|CONTROL|ok:|FAIL:|checks_run|probe_retained)' \
...
# ---------------------------------------------------------------------------
audit_rc=0
CALIPTRA_ROOT="${REPO_ROOT}" "${SCRIPT_DIR}/audit_kv_entry_clear_sources.sh" \
